...
Frontend development has changed since I started programming. You can't just turn up with some HTML, CSS and basic jQuery knowledge and make apps that *actually* meet customer expectations. Marketing sites and landing pages are only the tip of the iceberg for the average web developer now.

Before getting into frontend development I was writing scripts for my day job. After moving departments I got into SharePoint development, where I spent a fair amount of time writing jQuery selectors and messing about with DOM manipulation. Lots of .find() and .closest() calls, manually updating elements, that sort of thing.
